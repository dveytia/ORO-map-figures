...
fig1_A_ggp <- ggplot()+
  ...
  scale_y_continuous(sec.axis = sec_axis(transform = exp, name = "N articles",
...
```{r supplement - model temporal trends of ORO branches}
library(nlme)
library(mgcv)
...
## 2/ Fit individual models for each ORO branch
...
by_dataset_short <- by_dataset %>% filter(1975 <= year & year <= 2022)
modFits <- lapply(split(by_dataset_short, by_dataset_short$Dataset), gls_corAR1_fit)
...
## 3/ Test for difference in temporal trends compared to Ocean and Climate Science
...
sink(here::here("outputs/glsOROBranchAndScopusTemporalTrendModelSummary.txt"))
print(all_fit_sum)
print(trendCoeff)
sink()
...
by_dataset3 <- rbind(oro_branch_by_year_sums %>% 
                      rename(Dataset = ORO_branch, n_articles = mean) %>%
                      select(year, n_articles, Dataset),
  scopus_by_year %>% mutate(Dataset = paste("Ocean & Climate")),
  wos_by_year %>% mutate(Dataset = paste("WOS"))) %>%
  mutate(year = as.numeric(format(year, "%Y"))) %>%
  group_by(Dataset) %>%
  arrange(year) %>%
...
gamm_ggp <- plot_smooths(model = gammFit$gam, series = year, comparison = Dataset)+
  ...
  labs(y = "log(N articles)", x= "Year" 
       #caption = "All smoothing terms significant, shown on link scale\n Each line labelled with linear trend (1975 - 2022) quantified using gls model of the form:\nlog(N articles) ~ year * Dataset with correlation = corAR1(Year|Dataset)"
       )+
  ...
  scale_color_manual(values = c("Ocean & Climate"="black", "Mitigation"=varAES$colour[1],
                                "Natural resilience" = varAES$colour[2],
                                "Societal adaptation" = varAES$colour[3]),
                     name = "Dataset",
                     guide=guide_legend(order=2, nrow=2), position = "bottom")+
  scale_fill_manual(values = c("Ocean & Climate"="black", "Mitigation"=varAES$colour[1],
                                "Natural resilience" = varAES$colour[2],
                                "Societal adaptation" = varAES$colour[3]),
...
# Sum mitigation OROs with and without conference proceedings
oroTypeSums %>%
  filter(oro_type %in% c("Built infrastructure & technology",
                         "CO2 removal or storage",
                         "Marine renewable energy")) %>%
  group_by(conference_proceeding) %>%
  summarise(n_mean = sum(n_mean))
#   conference_proceeding n_mean
#   <lgl>                  <int>
# 1 FALSE                  27837
# 2 TRUE                    8853
...
# Summarise the percentage of mitigation vs adaptation
sum(oroMatchesProp_IPCC$prop_IPCC_ORO[oroMatchesProp_IPCC$oro_branch == "Mitigation"])*100 # 11.56337 %
sum(oroMatchesProp_IPCC$prop_IPCC_ORO[grep("Societ|Natural", oroMatchesProp_IPCC$oro_branch)])*100 # 88.43663 %

# And by ORO branch
oroMatchesProp_IPCC %>%
  group_by(oro_branch) %>%
  summarise(perc_ORO = sum(prop_IPCC_ORO)*100)
#   oro_branch          perc_ORO
#   <fct>                  <dbl>
# 1 Mitigation              11.6
# 2 Natural resilience      20.8
# 3 Societal adaptation     67.6
...
```{r Load aesthetics for branch interaction venn diagram}
## Load data
predRel <- tbl(dbcon, "pred_relevance") 
predType <- tbl(dbcon, "pred_oro_type_long") 
uniquerefs <- tbl(dbcon, "uniquerefs")
...
```{r plot the venn diagram of overlap between branches - ggVennDiagram}
## Calculate interaction matrix
oroBranchMat <- predTypeDf %>%
  filter(mean <= 0.5) %>% 
  distinct(analysis_id, oro_branch) %>% 
  mutate(presence = 1) %>%
  tidyr::pivot_wider(id_cols = analysis_id, names_from = oro_branch, values_from = presence, values_fill = 0)
summary(oroBranchMat)
anyDuplicated(oroBranchMat$analysis_id) # check no duplicates

## Check that the number of articles counted == the total number of relevant articles
sum(colSums(oroBranchMat[,2:3]))

## Format into list
vennList <- vector("list", length = ncol(oroBranchMat)-1)
names(vennList) <- colnames(oroBranchMat)[-1]
for(s in 1:length(vennList)){
  vennList[[s]] <- oroBranchMat$analysis_id[oroBranchMat[,s+1] == 1]
}
vennList <- Filter(function(x) length(x) > 0, vennList)

## Plot  
require(ggVennDiagram)
venn_ggp1 <- ggVennDiagram(vennList, label_alpha = 0)+
      ggplot2::scale_fill_distiller(name = "N articles", palette = "Blues", direction = 1)
...
# Check numbers
# Number of articles predicted relevant but not allocated an ORO type
nOther <- length(unique(predRel$analysis_id))-length(unique(oroBranchDf$analysis_id)) 
nOther #1435
# Number of branch duplicates (same article relevant to different ORO types within the same branch)
BranchDup <- oroBranchDf[duplicated(oroBranchDf),] 
nBranchDup <- length(unique(BranchDup$analysis_id))
nBranchDup # 2684
# without branch duplicates
noDup <- oroBranchDf[!duplicated(oroBranchDf),]
nnoDup <- length(unique(noDup$analysis_id))
nnoDup # 42758
# Check that I can arrive back at total number of relevant articles -- yes
length(unique(predRel$analysis_id)) # 44193
nnoDup + nOther # 44193




# Which papers are positive for all three branches?
# all Mitigation, but for two of them, I could see how keywords such as bio-productivity or "protection scheme" could flag relevance for other ORO types.
allBranchIds <- noDup$analysis_id[table(noDup$analysis_id) == 3]
# uniquerefs %>% 
#   filter(analysis_id %in% allBranchIds) %>%
#   collect() %>%
#   View() 
...
## Plot using GGVENN -- can allocate a different colour to each set
## Format into list
oroBranchInteractions_venn <- list(
  noDup$analysis_id[noDup$oro_branch == "Mitigation"],
  noDup$analysis_id[noDup$oro_branch == "Natural resilience"],
  noDup$analysis_id[noDup$oro_branch == "Societal adaptation"]
)
...
interactions_venn_ggp <- ggvenn(oroBranchInteractions_venn,
       ...
       fill_color = branchAES$colour, fill_alpha = 0.5,
       text_color = "black",
       stroke_size = 0.5, set_name_size = 4)

interactions_venn_ggp



## Plot using GGVENNDIAGRAM -- each set is coloured according to count
## Format into list
vennList <- vector("list", length = length(branchAES$label))
names(vennList) <- branchAES$label
for(s in 1:length(vennList)){
  vennList[[s]] <- noDup$analysis_id[noDup$oro_branch == branchAES$label[s]]
}
vennList <- Filter(function(x) length(x) > 0, vennList)
# Plot  
require(ggVennDiagram)
venn_ggp1 <- ggVennDiagram(oroBranchInteractions_venn, label_alpha = 0)+
      ggplot2::scale_fill_distiller(name = "N articles", palette = "Blues", direction = 1)
...
# Supplementary figure: Number of publications by scientific discipline
...
## Format and collect needed data
branchDisciplineDf <- predRel %>%
  filter(thresholdLevel <= relevance_mean) %>%
  select(analysis_id) %>%
  left_join(predBranch, by = "analysis_id") %>%
  mutate(Mitigation = ifelse(thresholdLevel <= `oro_branch.Mitigation - mean_prediction`,1,0),
         Adaptation = case_when(
           thresholdLevel <= `oro_branch.Nature - mean_prediction` ~ 1,
           thresholdLevel <= `oro_branch.Societal - mean_prediction` ~ 1,
           TRUE ~ 0
         )) %>%
  select(analysis_id, Mitigation, Adaptation) %>%
  filter(Mitigation != Adaptation) %>% # remove rows where mitigation and adaptation are the same -- would cancel out
  left_join(uniquerefs, by = "analysis_id") %>%
  select(analysis_id, Mitigation, Adaptation, research_areas, web_of_science_categories) %>%
  collect()

research_areas <- strsplit(branchDisciplineDf$research_areas, "; ")
research_areas <- data.frame(analysis_id = rep.int(branchDisciplineDf$analysis_id, sapply(research_areas, length)),
                             research_area = unlist(research_areas))
research_areas <- subset(research_areas, !is.na(research_area))

branchDisciplineDf <- research_areas %>%
  left_join(branchDisciplineDf %>% select(analysis_id, Mitigation, Adaptation), by = "analysis_id")


researchAreaTab <- branchDisciplineDf %>%
  group_by(research_area) %>%
  summarise(n_research_area = n()) %>%
  arrange(desc(n_research_area))

researchAreaTab %>% head(20)

write.csv(researchAreaTab, here::here("outputs/research_areas_tab.csv"))

researchAreaLookup <- read.csv(here::here("outputs/research_areas_tab.csv"), strip.white = TRUE, header = TRUE)
researchAreaLookup$X <- NULL
researchAreaLookup$n_research_area <- NULL

branchDisciplineDf <- branchDisciplineDf %>%
  left_join(researchAreaLookup, by = "research_area")

branchDisciplineDf <- branchDisciplineDf %>%
  filter(!is.na(Category)) %>%
  mutate(ORO_branch = ifelse(Mitigation == 1, "Mitigation","Adaptation")) %>%
  mutate(ORO_branch_f = factor(ORO_branch, levels = c("Mitigation","Adaptation")),
         Category_f = factor(Category, 
                             levels = c("Physical","Natural","Social"), 
                             labels = c("Physical Sciences","Natural and\nLife Sciences","Social Sciences")))

branchDisciplineSums <- branchDisciplineDf %>%
  group_by(ORO_branch_f, Category_f) %>%
  summarise(n = n_distinct(analysis_id))

research_area_ggp <- ggplot(branchDisciplineSums, aes(x = ORO_branch_f, y = n,label = n, fill = Category_f)) +
  geom_col()+
  geom_text(position = position_stack(vjust = 0.5))+
  scale_fill_brewer(type = "qual", palette = "Accent")+
  labs(y = "N Articles",x = "ORO branch", fill = "Research\area")+
  theme_bw()


ggsave(filename = here::here("figures/supplemental/nArticlesResearchAreaByOroBranch.pdf"),
       plot = research_area_ggp,
