...
```{r update for Laurent mCDR conference in Belgium 2025}

# Reproduce area plot but without deep ocean storage
timeseriesMethod2025_ggp <- ggplot(mCDR_type_year_tab %>%
                                     filter(
                                       mCDR_method != "Deep sea storage"
                                     ),
                               aes(x = year_date, y = n_articles, fill = mCDR_method)) +
  geom_area(colour = "black", size = .2) +
  scale_fill_brewer(palette = "Dark2")+
...
timeseriesMethod2025_ggp

ggsave(timeseriesMethod2025_ggp,
  filename = here::here("figures/mCDR_research_in_france_March2024/timeseriesMethod2025.png"),
