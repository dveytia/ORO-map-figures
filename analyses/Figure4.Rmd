...
## plot
fig4_B_percent_nonlinear_ggp <- ggplot(marSysTypeThreatSumsNorm, 
  ...
  #scale_x_discrete(labels = stringr::str_wrap(typeAES$label, width=20))+
...
fig4_C <- plot_grid(finalPlot, 
                    finalPlot2+draw_plot(biLegend, x=-0.23, y=0, width=0.4, height=0.4),
...
fig4_C_simplified <- ggplot()+
  ...
  geom_tile(data = mitigationSums %>% filter(0<n_articles & oro_branch == "Mitigation"), 
            aes(y=as.numeric(factor(oro_type)), x=as.numeric(factor(ecosystem_type)),fill = bi_class), show.legend = FALSE)+
  # Text of article numbers
  geom_text(data=mitigationSums %>% filter(0<n_articles & oro_branch == "Mitigation"),
            aes(y=as.numeric(factor(oro_type)), x=as.numeric(factor(ecosystem_type)),
...
plot_grid(fig4_A_ggp+theme(plot.margin = unit(c(0,0,2,0), "cm")),
          fig4_B_percent_nonlinear_ggp, 
          fig4_C_simplified2, #+theme(plot.margin = unit(c(2,0,0,0), "cm"))
