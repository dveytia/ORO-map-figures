  ...
  ## ---- FORMAT DATA
    ...
    mitigation_grid_df <- pred_blue_carbon %>%
      ...
      mean_GHGemi_country <- GHGemi_country |> 
        dplyr::select(year, iso_code, co2_including_luc) |>  # total_ghg refers to total GHG emissions (in MtCO2eq) including land-use change and forestry.
     ...
     GHGemi_country_allvar <- GHGemi_country |> 
        dplyr::select(year, iso_code, cumulative_co2, cumulative_co2_including_luc) |>  # total_ghg refers to total GHG emissions (in MtCO2eq) including land-use change and forestry.
      ...
      data_panelA <- MRE_BC_mean_eez_sf_noDOMTOM |> 
        sf::st_drop_geometry() |> 
        dplyr::select(ISO_SOV1, TERRITORY1, n_articles_weighted) |> 
        # rename(ISO_SOV1 = ISO_SOV1.x) |> 
        left_join(GHGemi_country_eez |> 
                    sf::st_drop_geometry() |> 
                    dplyr::select(ISO_SOV1, an_mean_co2eq_em, cumulative_co2), 
                  by = "ISO_SOV1") |> 
        distinct()
      ...
      data_panelA_sf <- eez_shp |> 
        dplyr::select(ISO_SOV1, TERRITORY1, geometry) |> 
...
### ----- PANEL B -----
  ...
  ## ---- FORMAT DATA
    ...
    # --- Univariate color scale: # pub of Adaptation OROs by 1st author affiliation
    ...
    # - Format the shapefile of the world countries polygon and bind data
    world_shp_boundaries_adapt <- format_shp_of_the_world(world_shp    = world_shp,
                                                          data_to_bind = adaptPubs_per_country,
                                                          PROJ         = 4326)
    
    # --- Bivariate color scale
    # --- Weighted # of articles relevant to adaptation per grid cell
      ...
      adapt_sf_eez2 <- eez_shp |> 
        left_join(adapt_sf_eez, by = "ISO_SOV1") 
      
      ggplot(adapt_sf_eez2) +
        geom_sf(data = world_shp, fill = "grey90") +
        geom_sf(aes(fill = n_articles_weighted)) +
        scale_fill_viridis_c() +
        theme_bw()
      
      # - Format vulnerability data (i.e., normalize values between 0 and 1)
      rast_ls <- list(mhvs   = mhvs_rast,
                      sst    = sst_rast,
                      storms = storms_rast,
                      surges = surges_rast) ; raster::plot(stack(rast_ls))
      ...
      raster::plot(vulne_mean_rast, colNA = "darkblue")
      raster::plot(vulne_mean_rast1, colNA = "darkblue")
      
      # - Shapefile and summarize across eez
      vulne_shp <- sf::st_as_sf(stars::st_as_stars(vulne_mean_rast1)) 
      vulne_shp_eez <- eez_shp |> 
        sf::st_join(vulne_shp) |> 
        sf::st_drop_geometry() |> 
        group_by(MRGID) |> 
        summarise(vulnerability = mean(layer, na.rm = TRUE))
      
      vulne_shp_eez2 <- eez_shp |> 
        left_join(vulne_shp_eez, by = "MRGID") 
      
      ggplot(vulne_shp_eez2) +
        geom_sf(data = world_shp, fill = "grey90") +
        geom_sf(aes(fill = layer)) +
        scale_fill_viridis_c() +
        theme_bw()
      
      # - Bind vulneralibity and adaptation paper data
      data_panelB <- adapt_sf_eez2 |> 
        sf::st_drop_geometry() |> 
        left_join(vulne_shp_eez2 |> 
                    sf::st_drop_geometry() |> 
                    dplyr::select(MRGID, vulnerability), 
                  by = "MRGID") |> 
        dplyr::select(MRGID, n_articles_weighted, vulnerability)
      ...
      # -- Adapt it to the data
      data_bivar_n_article_CO2em <- format_data_bivariate_map(data        = data_panelB,
                                                              data.x      = "n_articles_weighted",
      ...
      data_panelB_sf <- eez_shp |> 
        left_join(data_bivar_n_article_CO2em, by = "MRGID") 
      
      # -- Map 
      bivariate_map(data_map               = data_panelB_sf,
                    data_map_univ          = world_shp_boundaries_adapt,
                    data_world             = world_shp,
                    bivariate_color_scale  = bivariate_color_scale,
                    univariate_color_scale = viridis::rocket(10, direction = -1),
                    ylab                   = "Vulnerability",
                    xlab                   = "# Adaptation papers",
                    lab_univ               = "# adapt. paper (log)",
                    name                   = "main/Figure5_PanelB2")
