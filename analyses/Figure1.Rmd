...
fig1_A_ggp <- ggplot()+
  ...
  scale_color_manual(values = varAES$colour, 
                     name = "ORO branch", guide=guide_legend(order=1))+
  scale_fill_manual(values = factor_aes$colour[which(factor_aes$variable == "ORO_branch")], guide = "none")+
  scale_linetype_manual(values = c("solid","dotted"), name = "Oceans & climate\nliterature from\ndatabase:",
...
# keep to only one duplicate id in case there are multiple matches in the ipcc database?
ipccMatches <- ipccMatches[-duplicated(ipccMatches$duplicate_id),]
