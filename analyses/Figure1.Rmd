...
NB the analysis below is long so comment out
...
```{r identify matches between ipcc refs and our database, eval=FALSE}
# analagous to the find_duplicates function, group by year and look for doi matches
# and then if none, fuzzy title matching

# Source functions for analysis
source(here::here("R/utils.R"))
source(here::here("R/clean_string.R"))

# Load the unique ipcc references
load(here::here("data/external/IPCC-AR6-bibtex-files/IPCC_AR6_uniquerefs.RData"))

# subset the ipcc to the temporal range of our database
years <- as.numeric(uniquerefs$year)
#years <- years[!is.na(years)]
years <- unique(years)
years <- years[order(years)]

ipccBibUnique$year <- as.numeric(ipccBibUnique$year)
ipccBibUnique <- ipccBibUnique[(ipccBibUnique$year %in% years),]
nrow(ipccBibUnique) # 29952


# For each year in my dataset, calculate if there is a match
ipccMatchesList <- vector("list", length(years))

for(y in 1:length(years)){
  # subset data to that year
  if(is.na(years[y])){
    tempDf <- predTypeRefs[which(is.na(uniquerefs$year)),]
    if(sum(is.na(ipccBibUnique$year)) == 0){next} # if no matches, skip year
    ipccTempDf <- ipccBibUnique[which(is.na(ipccBibUnique$year)),]
  }else{
    tempDf <- predTypeRefs[which(uniquerefs$year == as.character(years[y])),]
    if(length(which(ipccBibUnique$year == years[y])) == 0){next} # if no matches, skip year
    ipccTempDf <- ipccBibUnique[which(ipccBibUnique$year == years[y]),]
  }
  ...
  myTitles <- tempDf$title[!is.na(tempDf$title)]
  ipccTitles <- ipccTempDf$title[!is.na(ipccTempDf$title)]
  if(length(myTitles) == 0 | length(ipccTitles) == 0){next}

  if(0 < sum(is.na(tempDf$ipccRef_id))){
  ...
  # save in list object
  ipccMatchesList[[y]] <- tempDf
}

# Bind all together into a data frame
ipccMatches <- do.call(rbind.data.frame, ipccMatchesList)
save(ipccMatchesList,
     file = here::here("data/external/IPCC-AR6-bibtex-files/IPCC_AR6_uniquerefs_matches.RData"))
```
