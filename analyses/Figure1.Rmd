...
for(b in 1:(length(thresholdBreaks)-1)){
  ...
  if(b == 1){
    tempDat <- thresholdSensitivityDf %>%
      group_by(oro_type) %>%
      summarise(
        n = sum(thresholdBreaks[b] <= relevance_mean & relevance_mean <= thresholdBreaks[b+1])
        ) %>%
...
oroDistOverThresholdWindow_ggp <- ggplot(data=thresholdSumsDf, aes(x=threshold_f, y=proportion))+
  ...
  geom_vline(xintercept = 5.5, col="red", linewidth = 1)+
...
```{r This could also be shown by a violin plot of the different relevance values for the different ORO types but it is more difficult to visualize the differences}

oroRelevanceViolinDf <- thresholdSensitivityDf 
...
typeAES <- typeAES[order(typeAES$order),]
# Use to factor oro_type column
oroRelevanceViolinDf$oro_type <- factor(
  oroRelevanceViolinDf$oro_type, 
  levels = typeAES$level, labels = typeAES$label
)

# Merge sums with plotting order, colour 
typeAES$oro_type <- typeAES$label
oroRelevanceViolinDf <- merge(oroRelevanceViolinDf,typeAES[,c("oro_type","order","colour")])
# oroRelevanceViolinDf$order <- factor(oroRelevanceViolinDf$order,
#                             levels = oroRelevanceViolinDf$order[order(oroRelevanceViolinDf$order)])



# Plot
oroRelevanceViolin_ggp <- ggplot(data=oroRelevanceViolinDf, aes(x=oro_type, y=relevance_mean))+
  geom_violin(aes(fill=oro_type))+
  geom_hline(yintercept = 0.5, col="red", linewidth = 1)+
  scale_fill_manual(values = typeAES$colour, name = "ORO type")+
  labs(x = "ORO type", y= "Relevance for inclusion (mean)")+
  theme_bw()+
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )

oroRelevanceViolin_ggp
...
for(b in 1:(length(thresholdBreaks)-1)){
  ...
  test <- chisq.test(x = tempDat$n, p = nullProp$prop)
...
oroDistOverThresholdWindow_ggp <- ggplot(data=thresholdSumsDf, aes(x=threshold_f, y=proportion))+
  geom_col(aes(fill=oro_type), position ="stack")+
  geom_text(aes(x=threshold_f, label = thresholdTotal), y=1.01, vjust = 0.5, hjust=0, check_overlap = TRUE, inherit.aes = FALSE, size=2, angle = 90)+
...
write.csv(thresholdChiSqDf, here::here("outputs/changingRelevanceThresholdSensitivityTest.csv"))

```

```{r plot figure of senstivity test results}
ggsave(
  here::here("figures/supplemental/changingRelevanceThresholdSensitivityTest.pdf"),
  plot = oroDistOverThresholdWindow_ggp +
  geom_text(data = thresholdChiSqDf %>% filter(chisq_pvalue < 0.05), aes(x=breakOrder-0.5),
            y=1.1, hjust=0, label = "*", inherit.aes = FALSE),
  width = 7, height = 5, units = "in"
)

```

```{r alternate version of figure but not as informative}
oroDistOverThreshold_ggp <- ggplot(data=thresholdChiSqDf, aes(x=as.factor(threshold), y=prop))+
  geom_col(aes(fill=oro_type), position ="stack")+
  # geom_text(aes(label = ), y=1.01, vjust = 0.5, hjust=0, check_overlap = TRUE, inherit.aes = FALSE, size=3, angle = 90)+
  geom_vline(xintercept = 5.5, col="red", linewidth = 1)+
  scale_fill_manual(values = typeAES$colour, name = "ORO type")+
  labs(x = "Threshold", y= "Proportion of articles")+
  ylim(0, 1.1)+
