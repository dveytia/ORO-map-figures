...
# 1              Scopus 0.07670911 0.07670911      1.079728 5.7e-08 * 
# 2          Mitigation 0.01103464 0.08774375      1.091708       0.57
# 3  Natural resilience 0.07652435 0.15323346      1.165597 0.00011 * 
# 4 Societal adaptation 0.06440174 0.14111085      1.151552  0.0019 *
...
gamm_ggp <- plot_smooths(model = gammFit$gam, series = year, comparison = Dataset)+
  ...
  geom_text(data = interannualTrendText %>%
              filter(Dataset != "Societal adaptation"), aes(x=year, y=log(n_articles), label = Value_abs_label),
...
ggsave(filename = "oroBranchTimeSeriesModelFits.svg",
       path = here::here("figures/supplemental"), device = 'svg',
...
ggsave(filename = "mitigationBranchInflectionYear.svg",
       path = here::here("figures/supplemental"), device = 'svg',
...
panelBText <- panelBDf %>%
  group_by(ORO_branch) %>%
  mutate(isOutlier = rstatix::is_outlier(derivative_est, coef = 2)) %>%
  filter(isOutlier, 0 < derivative_est, 1981 < year) %>%
...
## Save as supplementary file
ggsave(filename = "allBranchInflectionYear.svg",
       path = here::here("figures/supplemental"), device = 'svg',
...
fig1_ggp <- plot_grid(fig1_A_ggp+theme(legend.position = "bottom"), 
                      fig1_B_ggp+theme(legend.position = "bottom"),
          nrow=1, ncol=2, labels = c("a.","b."), align = "v", axis="b")

fig1_ggp

## Save as supplementary file
ggsave(filename = "oroBranchTimeAndIpccMatches.svg",
       path = here::here("figures/main"), device = 'svg',
