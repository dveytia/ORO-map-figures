...
### Note N°0: Select only empirical studies
  ...
  predMethod <- tbl(dbcon, "pred_method_type") |> 
      select(analysis_id, 
             `method_type.Empirical - mean_prediction`, `method_type.Social_primary - mean_prediction`) |> 
      filter(`method_type.Empirical - mean_prediction` >= 0.5 | `method_type.Social_primary - mean_prediction` >= 0.5) |> 
      collect()
...
### Note N°1 (filter 0): Select only data where !is.na()
...
### Note N°2 (filter 1): Capital letter filtering
...
### Note N°3 (filter 2): Remove word matching with copyright informations
...
### Note N°4 (filter 3): Remove land matches x distance from the coast when the spatial scale of the match is sub-national
...
### Note 6 (filter 5): Remove word matches that are a complete word in the English language
...
### Example: analysis_id == 264937
  ...
  for(i in 1:length(ana_id_unique)){
    ...
    # Keep only TRUE data if country name is identified in the title, else, keep all data
    analysis_id_i <- analysis_id_i |>  
  ...
  for(i in 1:length(ana_id_unique)){
    ...
    analysis_id_i <- suppressWarnings(analysis_id_i |> 
      mutate(country_in_kw  = case_when(stringr::str_detect(keywords, stringr::str_to_lower(TERRITORY1)) ~ TRUE,
                                        (!stringr::str_detect(keywords, stringr::str_to_lower(TERRITORY1)) & 
                                           sum(stringr::str_detect(keywords, paste0(stringr::str_to_lower(sea_with_country_name), collapse = "|")) == TRUE, na.rm=T) > 0) ~ TRUE,
                                        TRUE ~ FALSE),         
             country_in_kwo = case_when(stringr::str_detect(keywords_other, stringr::str_to_lower(TERRITORY1)) ~ TRUE,
                                        (!stringr::str_detect(keywords_other, stringr::str_to_lower(TERRITORY1)) & 
                                           sum(stringr::str_detect(keywords_other, paste0(stringr::str_to_lower(sea_with_country_name), collapse = "|")) == TRUE, na.rm=T) > 0) ~ TRUE,
                                        TRUE ~ FALSE),
             US_state_in_kw = case_when(stringr::str_detect(keywords, paste(stringr::str_to_lower(state.name), collapse = "|")) == TRUE & TERRITORY1 == "United States" ~ TRUE, 
                                        TRUE ~ FALSE)) |>
  tidyr::replace_na(list(country_in_kw = FALSE, country_in_kwo = FALSE, US_state_in_kw = FALSE)))
