...
uniqueWordPlaceMatches <- geoparsedText %>% select(word, place_name) %>% distinct(word, place_name, .keep_all = T)
nrow(uniqueWordPlaceMatches) # 8261
...
quantile(uniqueWordPlaceMatches$n_articles, probs = c(0,0.75, 0.8, 0.85, 0.9, 0.95, 1))

sum(uniqueWordPlaceMatches$n_articles[17 <= uniqueWordPlaceMatches$n_articles])
sum(uniqueWordPlaceMatches$n_articles[uniqueWordPlaceMatches$n_articles < 17])


highFreqMatches <- uniqueWordPlaceMatches %>%
