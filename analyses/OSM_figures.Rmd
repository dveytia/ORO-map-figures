...
grid_df_poly1 <- grid_df %>% 
  select(LON, LAT, grid_df_id) %>%
  mutate(LON = LON-(2.5/2), LAT = LAT-(2.5/2))
grid_df_poly2 <- grid_df %>% 
  select(LON, LAT, grid_df_id) %>%
  mutate(LON = LON-(2.5/2), LAT = LAT+(2.5/2))
grid_df_poly3 <- grid_df %>% 
  select(LON, LAT, grid_df_id) %>%
  mutate(LON = LON+(2.5/2), LAT = LAT+(2.5/2))
grid_df_poly4 <- grid_df %>% 
  select(LON, LAT, grid_df_id) %>%
  mutate(LON = LON+(2.5/2), LAT = LAT-(2.5/2))
...
ggsave(filename=here::here("testNbSmap.png"),plot=map_ggp,"png")
